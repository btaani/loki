...
To increase availability and reduce cross-zone bandwidth, `2x.medium-multizone` configures having 2 instances of ingesters and queriers in each zone if and only if zone-aware replication is enabled.
...
## Open Questions
* `lokiconfig.yaml` should be updated to include the `zone_awareness_enabled=true`. The `instance_availability_zone` will be different in the pods of differnt zones. 
...
* Peristent volumes are bound to the zone they are created in. How do we re-bind the restarted pod to the PV after a zone failure?
...
[] https://docs.openshift.com/container-platform/4.12/nodes/scheduling/nodes-scheduler-pod-topology-spread-constraints.html
[] https://cortexmetrics.io/docs/guides/zone-aware-replication/
